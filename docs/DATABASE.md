...
V1.7-5A地址模块后端补全未修改数据库结构，继续复用现有`user_address`字段。

默认地址规则：

- `is_default=1`表示默认地址。
- 同一用户最多一个默认地址，由业务层在新增、编辑和设置默认地址时先取消当前用户其它默认地址再写入目标地址。
- 地址编辑、删除和设置默认地址均按`user_id`与`is_deleted=0`校验归属，禁止操作他人地址。
...
V1.7-6用户小程序首页推荐门店复用本表公开查询结果，不新增表、不新增字段、不新增索引。
门店详情占位页仅读取店铺基础展示字段，商品列表与商品详情留到V1.7-7继续联调。
...
V1.7-7用户小程序商品分类导航复用本表公开查询结果，不新增表、不新增字段、不新增索引。
...
V1.7-7用户小程序商品列表和商品详情复用本表公开查询结果，不新增表、不新增字段、不新增索引。
公开商品列表只返回`product_status = 1`、`is_deleted = 0`、库存大于0，且分类、店铺和商家均处于可公开状态的数据。
...
- 商品上架要求分类启用且库存大于 0。
- 商品删除为逻辑删除，并同步设置为下架。
- V1.7-7A商品链路审计确认：公开商品分类、商品列表和商品详情均依赖
  `merchant_store.business_status = 1`。测试数据中若店铺为休息中，会导致门店详情正常但商品为空。
- 当前测试库已将`merchant_store.id=1`修正为`business_status=1`用于商品链路复测。
...
- 全选和取消全选只影响实时校验有效的购物车商品。
- 清理失效商品不新增表结构，仍通过 `is_deleted = 1` 进行逻辑删除。
...
- 未新增SQL文件。
- 未新增表、字段或索引。
- 商品链路审计发现测试数据`merchant_store.id=1`原为`store_status=1`、`business_status=0`。
- 公开商品分类和商品列表SQL要求`business_status=1`，因此分类和商品被过滤。
- 已修正当前测试库测试数据：`merchant_store.id=1`更新为`business_status=1`。
- 修正后`storeId=1`公开分类返回`categoryId=2`，公开商品列表返回`productId=2`。
...
- 新增幂等升级脚本`wuxin-paotui-server/src/main/resources/sql/15_update_rider_application.sql`。
- `rider_info`计划新增`reject_reason varchar(255) null`，用于记录审核拒绝或账号禁用原因。
- `rider_info`计划新增唯一索引`uk_rider_user_id(user_id)`，确保一个用户最多一条骑手资料并保护并发申请。
- 脚本通过`information_schema`检查字段和索引名称，可重复执行；不修改历史SQL，不更新业务数据。
- 2026-07-22用户已在Navicat中人工执行脚本；Codex没有重复执行DDL。
- 当前`rider_info`完整字段：`id`、`user_id`、`real_name`、`id_card`、`id_card_front`、`id_card_back`、`audit_status`、`rider_status`、`reject_reason`、`create_time`、`update_time`。
- 当前索引：`PRIMARY(id)`、`uk_rider_user_id(user_id)`，两者均为唯一索引。
- 收尾回归创建专用测试骑手`riderId=2`，最终状态为审核通过、正常启用、`reject_reason=NULL`；原骑手`riderId=1`保持正常。
- 数据库复核结果：2条骑手记录、无重复`user_id`、无孤立用户关联，原有骑手数据未被修改。
