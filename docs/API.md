...
V1.8时期`AdminRiderController`只提供四个写接口；V1.9已补充真实分页接口`GET /api/admin/rider/page`和详情接口`GET /api/admin/rider/{id}`。

## V1.9 总控后台接口

统一前缀为 `/api/admin`，必须携带 JWT，并通过 `@AdminPermission` 权限点校验。`ADMIN` 与 `SUPER_ADMIN` 保留超级权限。

| 模块 | 接口 |
| --- | --- |
| 会话 | `GET /api/admin/session` |
| Dashboard | `GET /api/admin/dashboard` |
| 订单 | `GET /api/admin/orders`、`GET /api/admin/orders/{id}`、`POST /api/admin/orders/{id}/cancel`、`POST /api/admin/orders/{id}/complete` |
| 用户 | `GET /api/admin/users`、`PUT /api/admin/users/{id}/status` |
| 骑手 | `GET /api/admin/rider/page`、`GET /api/admin/rider/{id}`；原审核、拒绝、启用、禁用接口继续保留 |
| 商品 | `GET /api/admin/products`、`GET /api/admin/products/categories`、`PUT /api/admin/products/{id}/status`、`PUT /api/admin/products/{id}/flags` |
| 财务 | `GET /api/admin/finance/summary` |
| 配置 | `GET /api/admin/configs`、`PUT /api/admin/configs/{id}` |
| 运营 | `/api/admin/operations/banners`、`/notices`、`/recommendations` 的查询、新增、编辑、删除 |
| RBAC | `GET /api/admin/rbac/roles`、`/permissions`、`/users`；`PUT /users/{id}/roles`、`PUT /roles/{id}/permissions` |
| 日志 | `GET /api/admin/logs` |
| 三端公共配置 | `GET /api/platform/home`，免登录，只返回启用且非敏感配置 |

V1.9 共新增 36 个 REST 入口。管理员取消已支付订单进入 `WAITING_REFUND(8)`；未支付订单进入取消。人工完成只允许履约状态 2、3。敏感配置查询统一返回 `******`，原掩码提交表示保持旧值。
