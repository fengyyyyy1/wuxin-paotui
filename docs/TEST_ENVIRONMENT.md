...
| 项 | 配置或结果 |
| --- | --- |
| 骑手端目录 | `wuxin-rider-app` |
| 商家端目录 | `wuxin-merchant-app` |
| 本地API | `http://localhost:8080` |
| 操作系统 | Windows 11 amd64 |
| Java | Temurin 21.0.11便携运行时 |
| Maven | Maven Wrapper 3.9.16 |
| Node / npm | Node.js 24.11.1 / npm 11.6.2 |
| 数据库 | MySQL 8，数据库名`wuxin_paotui` |
| 增量脚本 | `15_update_rider_application.sql`，用户已人工执行 |
| 骑手端检查 | Build、Lint、TypeScript通过 |
| 商家端检查 | Build、Lint、TypeScript通过 |
| 后端测试 | 42项通过，数据库密码仅通过本地环境变量注入 |
| 小程序AppID | 两端当前共用同一测试AppID，发布前必须分别配置 |

数据库升级后审计：7个有效用户、2名骑手、5个商家/店铺、1个有效商品、8个有效订单。`test001`对应`userId=2`、`riderId=1`、`merchantId=1`和`storeId=1`；本地Mock微信测试用户对应`userId=3`、`riderId=2`。

Navicat等价只读验证确认`reject_reason`、`PRIMARY(id)`和`uk_rider_user_id(user_id)`存在，骑手`user_id`无重复、无孤立关联。原骑手1数据正常；测试骑手2最终为审核通过、正常启用、拒绝原因为空。

HTTP回归通过骑手申请、重复申请、Profile、我的配送、订单详情、拒绝原因回写、拒绝后重申、审核通过、禁用、启用、重复启用、401和403。管理员骑手列表和详情GET接口当前不存在，探测返回业务码500，纳入V1.9总控端。

微信开发者工具验收前还需：

1. 为骑手端和商家端分别配置正式或测试AppID。
2. 本地阶段勾选不校验合法域名；上线前改为HTTPS合法域名。
3. 配置图片上传和正式COS HTTPS资源。
4. 继续真机验证商家接单/拒单/出餐与骑手接单/完成/放弃。
