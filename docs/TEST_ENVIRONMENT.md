...
| HTTP回归临时端口 | `8081`，回归结束后关闭 |
| Java | Temurin 21.0.11便携运行时 |
| 数据库 | 本地MySQL，只读审计，未执行迁移或业务写入 |
| 增量脚本 | `15_update_rider_application.sql`，尚待Navicat人工执行 |
| 骑手端检查 | Build、Lint、TypeScript通过 |
| 商家端检查 | Build、Lint、TypeScript通过 |
| 后端测试 | 41项通过，测试时显式设置`DB_PASSWORD=123456` |
| 小程序AppID | 两端暂为`touristappid`，发布前必须替换 |

数据库只读审计：7个有效用户、1名骑手、5个商家/店铺、1个有效商品、8个有效订单。`test001`对应`userId=2`、`riderId=1`、`merchantId=1`和`storeId=1`。

HTTP只读回归已通过商家资料/订单/详情/分类/商品以及骑手大厅/排行榜/统计。骑手资料和我的配送因数据库尚无`reject_reason`字段返回500，必须在人工执行15号脚本后复测。本轮未重放会改变订单状态的写流程。

微信开发者工具验收前还需：

1. 为骑手端和商家端分别配置正式或测试AppID。
2. 本地阶段勾选不校验合法域名；上线前改为HTTPS合法域名。
3. 人工执行15号脚本并复测骑手资料、申请、我的配送和管理员审核。
4. 验证商家接单/拒单/出餐与骑手接单/完成/放弃的防重复提交。

当前断点：V1.8骑手端与商家端开发完成，下一步进入总控端订单、用户、骑手与运营数据管理。
