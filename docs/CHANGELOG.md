...
- 修复用户端跑腿下单页仅提示“暂不能提交”的闭环断点，改为调用既有`POST /api/order/create`创建真实跑腿订单。
- 跑腿下单页进入时读取`GET /api/platform/home`，使用数据库配置的基础配送费、每公里费用、每公斤费用、夜间加价和起送金额计算预计费用。
- 支付服务支持跑腿订单创建JSAPI/Mock支付单，跑腿订单金额使用`order_info.price`，商品订单继续使用`total_amount`。
- Mock支付页识别Mock支付参数后调用`POST /api/payment/mock/{paymentNo}/success`，确保支付成功后回写订单支付状态、支付单状态和订单日志。
- 保持商品订单、购物车、商家接单、骑手接单、后台订单中心和统计接口不变。

### 联调状态

- 静态链路确认：商品订单状态流为用户创建并支付 -> 商家接单 -> 商家出餐 -> 骑手接单 -> 骑手完成 -> 用户确认 -> 后台统计。
- 静态链路确认：跑腿订单状态流为用户创建并支付 -> 骑手大厅 -> 骑手接单 -> 骑手完成 -> 用户确认 -> 后台统计。
- 本地真实HTTP/数据库联调仍被当前Shell缺少`DB_PASSWORD`阻塞，MySQL返回`Access denied for user 'root'@'localhost' (using password: NO)`；需注入本地数据库密码并确认16号SQL已执行后继续。
...
- 15号增量SQL已由用户人工执行，`reject_reason`和`uk_rider_user_id`结构验证通过；Codex未重复执行DDL
- 迁移后骑手申请、Profile、我的配送、订单详情、审核拒绝/通过、禁用/启用、拒绝原因回写、重复申请和重复操作均已通过真实HTTP回归
- 修复接单大厅缺少骑手身份校验的问题；非骑手、审核未通过或已禁用骑手返回业务码403
- Java 21环境下42项后端测试通过；骑手端与商家端Build、Lint、TypeScript检查通过
- 401未登录、403非管理员、403非有效骑手权限测试通过
- 管理员骑手列表和详情GET接口当前不存在，未伪造通过，留待V1.9总控端实现
- 骑手收益和商家财务营收没有真实专用接口，前端不伪造数据
- 两个新小程序当前共用同一测试AppID，尚需独立正式AppID、HTTPS合法域名、图片上传和微信开发者工具真机验收
- V1.8已有提交：`b488553`、`90ef758`、`ce703e7`、`a2770e0`；收尾提交使用中文标签
- 当前工作区仍保留用户端、购物车、地址及开发者工具配置的未提交修改，因此不满足`v1.8.0`标签的干净工作区条件
- 下一开发断点：V1.9总控端订单、用户、骑手、商家与运营数据管理
...
## V1.9 第一阶段 - 2026-07-22

### 增加功能

- 完成 11 个模块的企业运营总控台和 14 个后台路由页面。
- 新增 Dashboard 真实指标、趋势图、TOP10、最近订单和公告。
- 新增全量订单、用户、骑手、商品、运营、财务、配置、RBAC 与日志接口。
- 新增 16 号数据库增量脚本和 7 张总控业务表。
- 新增三端公共动态配置读取，用户端首页 Banner 改为数据库优先。

### 修复问题

- 修复 Admin Mapper 注解 SQL 中小于号未转义导致 Spring 启动失败的问题。
- 商品状态更新后改为主键精确回读，避免商品超过 100 条时误报不存在。
- RBAC 更新增加角色、权限有效性校验，并禁止当前管理员修改自己的角色。
- 修复商家审核入口、商家经营概览、跨模块订单与商品筛选联动。
