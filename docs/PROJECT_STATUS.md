...
- 用户端：微信登录入口、资料、手机号绑定、首页、跑腿入口、门店、商品、购物车、地址、下单、支付页、订单、个人中心、商家入驻、骑手申请和公益入口已完成代码侧闭环。
- 商家端：登录、入驻、审核状态、工作台、店铺、分类、商品、订单列表、订单详情、接单、拒单、出餐和经营概览已完成。
- 骑手端：登录、申请、审核状态、工作台、接单大厅、我的订单、订单详情、接单、完成配送、统计和排行榜已完成。
- 管理端：Dashboard、订单、用户、骑手、商家、商品、运营、财务、系统配置、权限和日志中心已完成第一阶段代码。
- 后端：Spring Boot 3、Java 21、MyBatis Plus、JWT、统一Result、统一异常、核心Mapper/Service/Controller和增量SQL 02-16 已存在。
- 数据库：用户、地址、订单、订单日志、商家、商品、购物车、支付流水、骑手申请、后台运营配置等结构已通过增量SQL沉淀。
- 配置中心：V1.9已提供后台配置和公共平台配置接口，三端可读取部分非敏感配置。
- 日志：订单日志、后台操作日志和部分业务审计日志已实现。
...
- Redis尚未接入，缺少验证码、限流、防刷、短时缓存和幂等锁等生产能力。
- 管理后台权限已具备第一阶段能力，但上线前仍需核对生产管理员账号、角色和菜单授权。
- 消息通知、订单状态订阅消息和客服触达能力仍需补齐或明确不上线范围。
- 商家端、骑手端需要独立正式AppID、真机验收和线上体验版审核。
- 后台配置数据库化已完成一部分，仍需逐项确认金额、距离、比例、客服、公告和首页运营项均可在线调整。

### P2 后续优化

- 高级统计、BI、多租户、复杂运营活动、自动调度、地图轨迹、复杂财务结算可推迟。
- Docker化、自动化CI/CD、灰度发布和监控告警可在最小可上线版本稳定后完善。

## V1.9 总控后台第一阶段状态

- 代码状态：Dashboard、订单、用户、骑手、商家、商品、运营、财务、系统配置、权限管理、日志中心已完成。
- 四端闭环：后台配置落库；用户、骑手、商家端启动时读取非敏感公共配置；用户首页 Banner 动态刷新。
- 权限：角色与权限点数据库化，Controller 注解校验，前端菜单与按钮同步按权限隐藏。
- 检查：Java 21 Compile 通过；Admin Build/Lint/TypeScript 通过；三个微信小程序 Build/Lint/TypeScript 通过。
- 自动测试：43项非数据库单元测试全部通过；完整44项中的唯一 `contextLoads` 因当前 Shell 未提供本地 MySQL 密码而无法连接数据库，且 Mapper 启动解析已通过。
- 待人工执行：16号SQL、V1.9新接口 Postman 与数据库回查、后台浏览器全流程验收。
- 四端闭环修复：跑腿下单页已调用真实`POST /api/order/create`；Mock支付已通过`POST /api/payment/mock/{paymentNo}/success`回写订单支付状态；支付服务已支持跑腿订单金额。

当前断点：V1.9-1 数据库迁移与总控后台真实接口联调。

包含：

- [x] 创建独立`wuxin-rider-app`和`wuxin-merchant-app`
- [x] 骑手端10个页面及真实认证、接单、配送、统计和排名链路
- [x] 商家端12个页面及真实入驻、订单、店铺、分类和商品管理链路
- [x] 新增骑手申请、资料、配送详情和管理员审核接口
- [x] Java 21后端42项自动化测试通过
- [x] 双端Build、Lint和TypeScript检查通过
- [x] 用户人工执行`15_update_rider_application.sql`
- [x] 骑手申请、资料、我的配送、订单详情及审核状态流转复测
- [x] 401、非管理员403、非有效骑手403权限复测
- [x] 修复接单大厅缺少有效骑手校验
- [ ] 管理员骑手列表和详情GET接口（当前Controller不存在，纳入V1.9总控端）
- [ ] 双端微信开发者工具真机验收和独立正式AppID配置
...
- [x] V1.7-8D Design System V1.0 与门店详情页 UI 全量重建完成代码侧改造
- [x] 用户端统一设计系统与22个页面整体重构
- [x] 登录、首页、搜索、门店、商品和购物车体验重建
- [x] 地址选择、结算预览、商品订单创建与订单详情真实接口联调
- [x] 微信JSAPI支付调用及支付中、成功、失败页面
- [x] 商家入驻真实接口表单、骑手申请边界说明与统一公益页面
- [x] 统一Loading、Skeleton、Empty、Error、Toast、Dialog和BottomSheet体验
- [x] 移除旧公益重复页面、旧空状态组件、测试头像和未使用Vant依赖
- [x] V2用户端全页面视觉回归修复：完成22个页面和5个公共组件的布局基线统一

### V2用户端全页面视觉回归修复

- 已审计`app.json`注册的22个页面，页面WXML、WXSS、TypeScript和JSON文件完整。
- 已修复首页、门店详情、商品详情、地址管理、个人中心和购物车的重点布局问题。
- 已统一横向Flex内容的`min-width`、`flex-shrink`、文本省略和金额不换行规则。
- 已统一固定底栏的安全区适配与页面底部内容预留，避免操作栏遮挡正文。
- 已清理Grid、负外边距、布局缩放和超宽计算等高风险布局写法。
- 已完成组件WXSS选择器审计，公共组件仅使用Class选择器。
- 请求层保留15秒超时，并提供用户可理解的超时错误信息。
- Build、Lint和TypeScript检查通过；微信开发者工具逐页视觉验收仍需人工复测。

### 用户端整体重构说明

- 小程序保持微信原生小程序和TypeScript，不修改后端、数据库或现有业务规则。
- 首页门店、热门商品、推荐商品均来自真实公开接口，不伪造评分、月售或配送费。
- 订单确认使用真实结算预览和购物车创建订单接口，支付使用现有微信JSAPI支付接口。
- 骑手线上申请已接入真实`POST /api/rider/apply`，审核与启停由管理员真实接口处理。
- 当前需要在微信开发者工具完成22个页面视觉、路由和真机交互验收。
- 真实微信支付仍依赖生产AppID、商户号、证书、回调和合法HTTPS域名。
...
该阶段历史断点：V1.7-8C 人工复测。

## 二十六、V1.7-8D Design System V1.0 与门店详情页 UI 全量重建

本阶段已完成代码侧改造：

- [x] 新增`miniprogram/styles/variables.wxss`
- [x] 新增`miniprogram/styles/foundation.wxss`
- [x] 新增`miniprogram/styles/typography.wxss`
- [x] 新增`miniprogram/styles/components.wxss`
- [x] `app.wxss`引入五鑫Design System V1.0
- [x] 原`.page`、`.card`、`.title`和`.subtitle`全局样式暂标记为legacy，避免影响现有页面
- [x] 门店详情页WXML/WXSS废弃旧结构后从空文件重建
- [x] 新门店详情页使用`store-hero`、`store-toolbar`、`store-products`、`store-cart-bar`、`cart-sheet-mask`和`cart-sheet`根级结构
- [x] 新搜索框、分类栏、商品卡、数量控制器、底部购物车栏和BottomSheet使用Design System视觉规范
- [x] 新增自绘SVG搜索图标和购物车图标，无第三方授权依赖
- [x] 保留原有门店详情、商品列表、搜索、分类筛选、购物车、图片兜底、登录和异常处理逻辑

本阶段尚未完成：

- [ ] 微信开发者工具人工视觉验收
- [ ] 首页UI重建
- [ ] 独立购物车页按新Design System重建
- [ ] V1.7-9提交订单
...
- 骑手端10个页面、商家端12个页面完成。
- 新增骑手申请、Profile、订单详情及管理员审核、拒绝、启用、禁用7个接口。
- 用户已人工执行`15_update_rider_application.sql`；`rider_info.reject_reason`和`uk_rider_user_id`验证通过。
- 骑手申请、重复申请、Profile、我的配送、订单详情、拒绝原因、重申、审核通过、禁用和启用真实HTTP回归通过。
- 非骑手和禁用骑手访问接单大厅返回业务码403；未登录返回HTTP 401；非管理员访问总控骑手接口返回HTTP 403。
- 原骑手`riderId=1`保持正常；专用回归骑手`riderId=2`最终审核通过、正常启用。
- 管理员骑手列表和详情GET接口当前不存在，进入V1.9总控端范围。
- 已有V1.8提交：`b488553`、`90ef758`、`ce703e7`、`a2770e0`；本次另建中文收尾提交。
- 工作区仍保留用户端、购物车、地址及微信开发者工具配置的未提交修改，不纳入V1.8收尾提交。
- 上线前仍需独立正式AppID、HTTPS合法域名、图片上传、生产微信配置和真机验收。
