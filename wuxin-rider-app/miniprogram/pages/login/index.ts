...
Page({
  ...
  async onPasswordLogin() {
    if (!this.data.username || !this.data.password || this.data.submitting) { wx.showToast({ title: '请输入账号和密码', icon: 'none' }); return; }
    this.setData({ submitting: true });
    try { await loginWithPassword({ username: this.data.username, password: this.data.password }); }
