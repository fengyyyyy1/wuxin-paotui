...
public interface AdminMerchantMapper {
    ...
    @Select({
            ...
            "(SELECT COUNT(*) FROM merchant_product p WHERE p.store_id = s.id AND p.is_deleted = 0) AS productCount,",
            "(SELECT COUNT(*) FROM order_info o WHERE o.store_id = s.id AND o.deleted = 0) AS orderCount,",
            "(SELECT COALESCE(SUM(COALESCE(o.total_amount, o.price, 0)), 0) FROM order_info o WHERE o.store_id = s.id AND o.deleted = 0 AND o.pay_status = 1) AS revenueAmount",
            "FROM merchant_info m",
