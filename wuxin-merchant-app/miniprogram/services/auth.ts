...
export async function loginWithWechat(): Promise<void> { const useMock = IS_DEVELOPMENT_ENV && wx.getStorageSync<boolean>(MOCK_WECHAT_LOGIN_STORAGE_KEY) === true; const code = useMock ? 'mock-code-test001' : await new Promise<string>((resolve, reject) => wx.login({ success: r => r.code ? resolve(r.code) : reject(new Error('未获取到微信登录凭证')), fail: reject })); const result = await wechatLogin(code); saveAuth(result.token, result.userInfo); restoreSession(); await routeByIdentity(); }
